...
class YoloFunctionWrapper(yolo_pb2_grpc.YoloTensorflowWrapperServicer):
    ...
    def image_decode__image(self, request, context):
        ...
        with tf.name_scope(_id), Global_Graph_Dict[_id].as_default():
            ...
            print(f'misun: image_raw={image_raw}, obj_id={obj_id}')
    ...
    def image_resize(self, request, context):
        ...
        with tf.name_scope(_id), Global_Graph_Dict[_id].as_default():
            ...
            size = request.size
            print(f'misun: image={image}, obj_id={image_id}')
            print('misun: image=', type(image), 'len=', len(image), 'tensor_shape=', image.shape, 'image[0].shape=', image[0].shape, 'size=', request.size)
